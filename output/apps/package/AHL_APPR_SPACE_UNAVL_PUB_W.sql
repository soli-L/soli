  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."AHL_APPR_SPACE_UNAVL_PUB_W" AUTHID CURRENT_USER as
  /* $Header: AHLWSUAS.pls 115.5 2003/11/04 10:54:00 rroy noship $ */
  procedure rosetta_table_copy_in_p1(t out nocopy ahl_appr_space_unavl_pub.space_restriction_tbl, a0 JTF_NUMBER_TABLE
    , a1 JTF_DATE_TABLE
    , a2 JTF_NUMBER_TABLE
    , a3 JTF_DATE_TABLE
    , a4 JTF_NUMBER_TABLE
  ...
  procedure rosetta_table_copy_out_p1(t ahl_appr_space_unavl_pub.space_restriction_tbl, a0 out nocopy JTF_NUMBER_TABLE
...
end ahl_appr_space_unavl_pub_w;

CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_APPR_SPACE_UNAVL_PUB_W" as
  /* $Header: AHLWSUAB.pls 115.4 2003/11/04 10:53:58 rroy noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).space_unavailability_id := a0(indx);
          ...
          t(ddindx).org_name := a8(indx);
          t(ddindx).department_id := a9(indx);
          t(ddindx).dept_description := a10(indx);
          t(ddindx).space_id := a11(indx);
          t(ddindx).space_name := a12(indx);
          t(ddindx).start_date := rosetta_g_miss_date_in_map(a13(indx));
          t(ddindx).end_date := rosetta_g_miss_date_in_map(a14(indx));
  ...
  if t is null or t.count = 0 then
    ...
    a32 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).space_unavailability_id;
          ...
          a8(indx) := t(ddindx).org_name;
          a9(indx) := t(ddindx).department_id;
          a10(indx) := t(ddindx).dept_description;
          a11(indx) := t(ddindx).space_id;
          a12(indx) := t(ddindx).space_name;
          a13(indx) := t(ddindx).start_date;
          a14(indx) := t(ddindx).end_date;
  ...
  as
    ddp_x_space_restriction_tbl ahl_appr_space_unavl_pub.space_restriction_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ahl_appr_space_unavl_pub_w.rosetta_table_copy_in_p1(ddp_x_space_restriction_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
    ...
    ahl_appr_space_unavl_pub.process_space_restriction(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      p_module_type,
      ddp_x_space_restriction_tbl,
      x_return_status,
      x_msg_count,
      x_msg_data);
    ...
    ahl_appr_space_unavl_pub_w.rosetta_table_copy_out_p1(ddp_x_space_restriction_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
